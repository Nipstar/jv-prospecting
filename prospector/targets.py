...
EXPORT_COLUMNS = [
    ...
    "flags", "company_number", "years_trading", "tps_checked",
]
...
def list_targets(conn: sqlite3.Connection, min_score: int = 0, run_id: int | None = None) -> list[sqlite3.Row]:
    query = (
...
def export_targets_csv(conn: sqlite3.Connection, out_path: Path | None = None, min_score: int = 0, run_id: int | None = None) -> Path:
